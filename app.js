(() => {
  ...
  const defaultState = { people: [], records: [], settings: { cuttingsPerTray: 130, duplicateSeconds: 5, dailyGoal: 5, syncUrl: '', autoSync: false } };
  let state = loadState();
  let cameraStream = null;
  let cameraLoop = null;
  const $ = id => document.getElementById(id);
  const els = {};
  const ids = ['networkDot','networkText','todayLabel','scanForm','scanCode','origin','trayCount','registerFeedback','todayTrays','todayCuttings','todayPeople','pendingCount','recentRecords','personForm','personName','personId','personCode','peopleImport','peopleTotal','peopleTable','peopleEmpty','summaryPeriod','metricTrays','metricCuttings','metricPeople','metricAverage','goalLabel','personBars','recordsTable','recordsEmpty','exportButton','settingsForm','cuttingsPerTray','duplicateSeconds','dailyGoal','syncForm','syncUrl','autoSync','syncButton','syncStatus','clearRecordsButton','cameraButton','cameraDialog','cameraVideo','cameraStatus','closeCamera','qrDialog','qrPersonName','qrPersonMeta','qrCanvas','closeQr','printQr','toast'];
  ...
  async function registerScan(rawValue) {
    ...
    const record={id:crypto.randomUUID(),timestamp:now.toISOString(),personId:person.id,codigo:person.codigo,cedula:person.cedula,persona:person.nombre,trays,cuttings,origin:els.origin.value,synced:false};
  ...
  function fillSettings(){els.cuttingsPerTray.value=state.settings.cuttingsPerTray;els.duplicateSeconds.value=state.settings.duplicateSeconds;els.dailyGoal.value=state.settings.dailyGoal;els.syncUrl.value=state.settings.syncUrl;els.autoSync.checked=Boolean(state.settings.autoSync)}
  ...
  function importPeople(file){
    const reader=new FileReader();reader.onload=()=>{const rows=parseCsv(String(reader.result));if(rows.length<2){toast('El CSV no contiene datos');return}const headers=rows[0].map(h=>clean(h).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase());const nameIndex=headers.findIndex(h=>h==='nombre'||h==='persona');const idIndex=headers.findIndex(h=>h==='cedula'||h==='documento');const codeIndex=headers.findIndex(h=>h==='codigo');if(nameIndex<0||idIndex<0||codeIndex<0){toast('Faltan las columnas Nombre, Cédula o Código');return}let added=0;rows.slice(1).forEach(cols=>{const nombre=clean(cols[nameIndex]),cedula=clean(cols[idIndex]),codigo=clean(cols[codeIndex]);if(!nombre||!cedula||!codigo)return;if(state.people.some(p=>normalized(p.cedula)===normalized(cedula)||normalized(p.codigo)===normalized(codigo)))return;state.people.push({id:crypto.randomUUID(),nombre,cedula,codigo});added++});saveState();renderPeople();toast(`${added} persona${added===1?'':'s'} importada${added===1?'':'s'}`)};reader.readAsText(file,'utf-8');
  }
  function exportCsv(){
  ...
  els.summaryPeriod.addEventListener('change',renderSummary);els.exportButton.addEventListener('click',exportCsv);
